...
-- NEW STEPS (TO ALLOW DIAGONAL MOVES)
-- Step to the bottom left.
stepBottomLeft :: Cell -> Step 
stepBottomLeft c = (c, cellBottom (cellLeft c))

-- Step to the bottom right.
stepBottomRight :: Cell -> Step 
stepBottomRight c = (c, cellBottom (cellRight c))

-- Step to the top left.
stepTopLeft :: Cell -> Step 
stepTopLeft c = (c, cellTop (cellLeft c))

-- Step to the top right.
stepTopRight :: Cell -> Step 
stepTopRight c = (c, cellTop (cellRight c))

-- All steps. (UPDATED)
allStepsFrom :: Cell -> [Step]
allStepsFrom c = [stepLeft c, stepRight c, stepTop c, stepBottom c, 
                  stepBottomLeft c, stepBottomRight c, stepTopLeft c, stepTopRight c]

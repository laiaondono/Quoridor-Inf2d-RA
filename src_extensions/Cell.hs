...
-- NEW CELLS (TO ALLOW DIAGONAL MOVES)
-- Cell on the bottom left.
cellBottomLeft :: Cell -> Cell 
cellBottomLeft (i, j) = (columnLeft i, j - 1)

-- Cell on the bottom right.
cellBottomRight :: Cell -> Cell 
cellBottomRight (i, j) = (columnRight i, j - 1)
...
-- All surrounding cells. (UPDATED)
cellsAround :: Cell -> [Cell]
cellsAround c = [cellLeft c, cellRight c, cellTop c, cellBottom c, 
                 cellBottomLeft c, cellBottomRight c, cellTopLeft c, cellTopRight c]
...
-- 'Taxicab' distance between cells. (UPDATED)
...
-- NEW (TO ALLOW DIAGONAL MOVES)
-- Two cells are next to each other in the same diagonal.
isDiagonallyAdjacent :: Cell -> Cell -> Bool 
isDiagonallyAdjacent c c' = (c == cellBottomLeft c') || (c == cellBottomRight c') || (c == cellTopLeft c') || (c == cellTopRight c')
